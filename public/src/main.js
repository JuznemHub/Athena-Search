...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function loadStorageForm() {
    ...
    try {
      ...
      // Self-hosted runs on its local database only; there is no provider to pick.
      if (data.github_available === false) {
        // Use the label the backend reports, so this cannot drift from the
        // database actually in use.
        const label = data.store_label || 'Self-hosted database';
        if (storageProvider) {
          storageProvider.innerHTML = `<option value="local">${escapeHtml(label)}</option>`;
          storageProvider.value = 'local';
          storageProvider.disabled = true;
        }
        githubFields?.classList.add('hidden');
        if (saveStorageBtn) saveStorageBtn.classList.add('hidden');
        if (syncStorageBtn) syncStorageBtn.classList.add('hidden');
        if (storageStatus) {
          storageStatus.innerHTML =
            `<span style="color:var(--success-color)">${escapeHtml(data.db_engine || label)} · backups go to Telegram</span>`;
        }
        if (storageDesc) {
          storageDesc.textContent =
            `Self-hosted: links live in your own ${data.db_engine || 'database'}. Backups are sent to Telegram (and Drive if configured).`;
        }
        return;
      }

      if (storageProvider) storageProvider.value = provider;
      updateStorageSyncLabel(provider);
      ...
      if (storageDesc) {
        storageDesc.textContent = provider === 'github'
          ? 'Links live as Markdown in your repo. GitHub is the source of truth; D1 only caches for fast search.'
          : 'Links live in Cloudflare D1. Switch to GitHub to own them as Markdown.';
      }
